...
anacional <- function(directorio="/Users/nataliaarteaga/Documents/DANE/Procesos DIMPE /PilotoEMMET",
  ...
  anexo_nacional<-function(data,mes,year){
    # 1. Var y cont_Anual -----------------------------------------------------

    contribucion_total <- data %>%
      filter(MES==which(meses==mes) & ANIO%in%c(year-1)) %>%
    ...
    contribucion <- data %>%
      filter(MES==which(meses==mes) & ANIO%in%c(year,year-1)) %>%
    ...
    tabla1 <- data %>%
      filter(ANIO%in%c(year,year-1) & MES%in%which(meses==mes)) %>%
      group_by(ANIO,MES,DOMINIO_39,DESCRIPCIONDOMINIOEMMET39) %>%
    ...
    tabla1[paste0("varprodnom_",year)] <- (tabla1[paste0("produccionNom_",year)]-tabla1[paste0("produccionNom_",year-1)])/tabla1[paste0("produccionNom_",year-1)]
    tabla1[paste0("varprod_",year)] <- (tabla1[paste0("produccion_",year)]-tabla1[paste0("produccion_",year-1)])/tabla1[paste0("produccion_",year-1)]
    tabla1[paste0("varventasnom_",year)]<- (tabla1[paste0("ventasNom_",year)]-tabla1[paste0("ventasNom_",year-1)])/tabla1[paste0("ventasNom_",year-1)]
    tabla1[paste0("varventas_",year)]<- (tabla1[paste0("ventas_",year)]-tabla1[paste0("ventas_",year-1)])/tabla1[paste0("ventas_",year-1)]
    tabla1[paste0("varpersonas_",year)] <- (tabla1[paste0("personas_",year)]-tabla1[paste0("personas_",year-1)])/tabla1[paste0("personas_",year-1)]
    ...
    tabla1 <- inner_join(x=tabla1,y=contribucion,by=c("DOMINIO_39"))
    tabla1 <- tabla1[,c("DOMINIO_39","DESCRIPCIONDOMINIOEMMET39.x","varprodnom",
    ...
    Enunciado<-paste0(mes,"(",year,"/",year-1,")")
    ...
    tabla1[paste0("varventas_",year)]<- (tabla1[paste0("ventas_",year)]-tabla1[paste0("ventas_",year-1)])/tabla1[paste0("ventas_",year-1)]
    tabla1[paste0("varpersonas_",year)] <- (tabla1[paste0("personas_",year)]-tabla1[paste0("personas_",year-1)])/tabla1[paste0("personas_",year-1)]
    tabla1[paste0("varempleo_",year)] <- (tabla1[paste0("empleo_",year)]-tabla1[paste0("empleo_",year-1)])/tabla1[paste0("empleo_",year-1)]
    tabla1[paste0("varemptem_",year)] <- (tabla1[paste0("emptem_",year)]-tabla1[paste0("emptem_",year-1)])/tabla1[paste0("emptem_",year-1)]
    tabla1[paste0("varempleados_",year)] <- (tabla1[paste0("empleados_",year)]-tabla1[paste0("empleados_",year-1)])/tabla1[paste0("empleados_",year-1)]
    tabla1[paste0("varoperarios_",year)] <- (tabla1[paste0("operarios_",year)]-tabla1[paste0("operarios_",year-1)])/tabla1[paste0("operarios_",year-1)]
    tabla1[paste0("varsueldos_",year)] <- (tabla1[paste0("sueldos_",year)]-tabla1[paste0("sueldos_",year-1)])/tabla1[paste0("sueldos_",year-1)]
    tabla1[paste0("varsuelemplper_",year)] <- (tabla1[paste0("suelemplper_",year)]-tabla1[paste0("suelemplper_",year-1)])/tabla1[paste0("suelemplper_",year-1)]
    tabla1[paste0("varsuelempltem_",year)] <- (tabla1[paste0("suelempltem_",year)]-tabla1[paste0("suelempltem_",year-1)])/tabla1[paste0("suelempltem_",year-1)]
    tabla1[paste0("varsueltotemp_",year)] <- (tabla1[paste0("sueltotemp_",year)]-tabla1[paste0("sueltotemp_",year-1)])/tabla1[paste0("sueltotemp_",year-1)]
    tabla1[paste0("varsueltotope_",year)] <- (tabla1[paste0("sueltotope_",year)]-tabla1[paste0("sueltotope_",year-1)])/tabla1[paste0("sueltotope_",year-1)]
    tabla1[paste0("varhoras_",year)] <- (tabla1[paste0("horas_",year)]-tabla1[paste0("horas_",year-1)])/tabla1[paste0("horas_",year-1)]
    ...
    contribucion_total <- data %>%
      filter(MES%in%c(1:which(meses==mes)) & ANIO%in%c(year-1)) %>%
      summarise(produccion_total = sum(PRODUCCIONREALPOND),
                ventas_total=sum(VENTASREALESPOND),
                personal_total=sum(TOTALEMPLEOPERMANENTE+TOTALEMPLEOTEMPORAL+TOTALEMPLEOADMON+TOTALEMPLEOPRODUC))

    contribucion <- data %>%
      filter(MES%in%c(1:which(meses==mes)) & ANIO%in%c(year,year-1)) %>%
      mutate(PERSONAL=TOTALEMPLEOPERMANENTE+TOTALEMPLEOTEMPORAL+TOTALEMPLEOADMON+TOTALEMPLEOPRODUC) %>%
      group_by(ANIO,DOMINIO_39,DESCRIPCIONDOMINIOEMMET39) %>%
      summarise(prod = sum(PRODUCCIONREALPOND),vent=sum(VENTASREALESPOND),
                per=sum(PERSONAL)) %>% group_by(DOMINIO_39,DESCRIPCIONDOMINIOEMMET39) %>%
      summarise(produccion=(prod[2]-prod[1])/contribucion_total$produccion_total,
                ventas=(vent[2]-vent[1])/contribucion_total$ventas_total,
                personal=(per[2]-per[1])/contribucion_total$personal_total) %>%
      arrange(produccion)
    ...
    tabla1 <- data %>%
      filter(ANIO%in%c(year,year-1) & MES%in%c(1:which(meses==mes))) %>%
      group_by(ANIO,DOMINIO_39,DESCRIPCIONDOMINIOEMMET39) %>%
      summarise(produccionNom=sum(PRODUCCIONNOMPOND),
                produccion=sum(PRODUCCIONREALPOND),
                ventasNom = sum(VENTASNOMINPOND),
    ...
    Enunciado<-paste0(meses[1],"-",mes,"(",year,"/",year-1,")")
    addDataFrame(data.frame(Enunciado), sheet, col.names=FALSE, row.names=FALSE,
                 startRow = 9, startColumn = 1)

    # 5. Var y cont_doce meses ------------------------------------------------

    data$ANIO2 <- as.numeric(ifelse(data$MES%in%c((which(meses==mes)+1):12),data$ANIO+1,data$ANIO))
    contribucion_total <- data %>%
    ...
    Enunciado<-paste0(meses[12]," ",year-1,"-",mes," ",year,"/",meses[12]," ",year-2,"-",mes," ",year-1)
    addDataFrame(data.frame(Enunciado), sheet, col.names=FALSE, row.names=FALSE,
                 startRow = 9, startColumn = 1)
    ...
    Enunciado<-paste0(meses[1]," ",year-4,"-",mes," ",year)
    addDataFrame(data.frame(Enunciado), sheet, col.names=FALSE, row.names=FALSE,
      ...
      mutate(DESCRIPCIONDOMINIOEMMET39_ENLACE=
               ifelse(
                 ...
                 ifelse(DOMINIO_39_ENLACE==1089 , "Resto de alimentos",
                        ifelse(DOMINIO_39_ENLACE==1400 , "Confección de prendas de vestir",
                               ifelse(DOMINIO_39_ENLACE==2020 , "Fabricación de otros productos químicos",
                                      ifelse(DOMINIO_39_ENLACE==1900 , "Refinación de petróleo",DESCRIPCIONDOMINIOEMMET39))))))
    ...
    CALENDAR.FN <- function(From_year,To_year){
      ...
      # Incluir los días festivos que no se mueven cuándo caen en fin de semana + antes de 2011
      ...
      # Incluir domingo de ramos y domingo de resurrección
    ...
    calendar<-as.data.frame(CALENDAR.FN(2001,2022))
    ...
    deses<-tabla1[,c("DOMNOS","CLASESNDUSTRALES","ANO","MES","PRODUCCONREAL")]
    ...
    contribucion_total <- data %>%
      filter(MES==which(meses==mes) & ANIO%in%c(2019)) %>%
      summarise(produccion_total = sum(PRODUCCIONREALPOND),
                ventas_total=sum(VENTASREALESPOND),
                personal_total=sum(TOTALEMPLEOPERMANENTE+TOTALEMPLEOTEMPORAL+TOTALEMPLEOADMON+TOTALEMPLEOPRODUC))

    contribucion <- data %>%
      filter(MES==which(meses==mes) & ANIO%in%c(year,2019)) %>%
      mutate(PERSONAL=TOTALEMPLEOPERMANENTE+TOTALEMPLEOTEMPORAL+TOTALEMPLEOADMON+TOTALEMPLEOPRODUC) %>%
      group_by(ANIO,MES,DOMINIO_39,DESCRIPCIONDOMINIOEMMET39) %>%
      summarise(prod = sum(PRODUCCIONREALPOND),
                vent=sum(VENTASREALESPOND),
    ...
    tabla1 <- data %>%
      filter(ANIO%in%c(year,2019) & MES%in%which(meses==mes)) %>%
      group_by(ANIO,DOMINIO_39,DESCRIPCIONDOMINIOEMMET39) %>%
      summarise(produccionNom=sum(PRODUCCIONNOMPOND),
                produccion=sum(PRODUCCIONREALPOND),
                ventasNom = sum(VENTASNOMINPOND),
    ...
    tabla1 <- tabla1 %>%
      pivot_wider(names_from = c("ANIO"),values_from = c("produccionNom","produccion","ventasNom","ventas","personas"))
    tabla1[paste0("varprodnom_",year)] <- (tabla1[paste0("produccionNom_",year)]-tabla1[paste0("produccionNom_",2019)])/tabla1[paste0("produccionNom_",2019)]
    tabla1[paste0("varprod_",year)] <- (tabla1[paste0("produccion_",year)]-tabla1[paste0("produccion_",2019)])/tabla1[paste0("produccion_",2019)]
    tabla1[paste0("varventasnom_",year)]<- (tabla1[paste0("ventasNom_",year)]-tabla1[paste0("ventasNom_",2019)])/tabla1[paste0("ventasNom_",2019)]
    tabla1[paste0("varventas_",year)]<- (tabla1[paste0("ventas_",year)]-tabla1[paste0("ventas_",2019)])/tabla1[paste0("ventas_",2019)]
    tabla1[paste0("varpersonas_",year)] <- (tabla1[paste0("personas_",year)]-tabla1[paste0("personas_",2019)])/tabla1[paste0("personas_",2019)]
