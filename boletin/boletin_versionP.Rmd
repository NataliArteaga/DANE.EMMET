...
data<-read.csv(paste0(params$directorio,"/results/S5_tematica/EMMET_PANEL_tematica_",meses[params$month],params$year,".csv"),fileEncoding = "latin1")
...
tabla1[40,c("varprod","varventas","varpersonas")]<-0.001
...
tabla_paginada[[i]] <- tabla_paginada[[i]] %>% gt(rowname_col="Clase",groupname_col = "") %>% cols_width(
    DESCRIPCIONDOMINIOEMMET39 ~ pct(18),
    varprod_plot ~  pct(5),
...
\textbf{`r paste0(meses_b[params$month]," ",params$year,"/",params$year-1,"P")`}\
\
En `r paste0(tolower(meses_b[params$month])," de ",params$year)` frente a `r paste0(tolower(meses_b[params$month])," de ",params$year-1)` de los 10 dominios de ciudades representados por la encuesta, `r sum(tabla1$varprod>0)` registraron variaciones positivas en su producción real, sumando `r sum(tabla1$produccion[tabla1$varprod>0])` p.p. de la variación total nacional (Tabla 6)\
