...
data<-read_xlsx("D:/Documentos/CAMILA/DANE/EMMET Base Temática definitiva Nov2022....xlsx")
...
ggplot(variaciones,aes(x=variable,y=variacion,fill=variable))+geom_bar(stat="identity")+
  ...
  geom_text(aes(x=variable,y=variacion,label=paste0(round(variacion*100,1),"%")),vjust=-0.5)+
...
```{r historico,echo=F,eval=TRUE,warning=FALSE,message=F,fig.height=3,fig.width=5}
#2
...
bar_chart <- function(label, height = "16px", fill = "#00bfc4", background = "white") {
    ...
    chart <- glue::glue("<div style='flex-grow:1;margin-left:2px;background:{background};'>{bar}</div>")
...
tabla_paginada[[i]] <- tabla_paginada[[i]] %>% gt(rowname_col="Clase",groupname_col = "") %>% cols_width(
    DESCRIPCIONDOMINIOEMMET39 ~ px(110),
    varprod_plot ~  px(30),
    ...
    everything() ~ px(50)
                     ...
                     personal =md("**Contribución<br>Personas**")) %>% tab_options(table.font.size=5) %>% tab_style(style=cell_text(color="white"),
                                                                                                                    locations=cells_body(rows=varprod==0))
...
## Variación año corrido (%) y contribución, del valor de la producción, ventas y empleo total, segúun clase industrial {.unlisted }
...
```{r anio_corrido_view,echo=F,eval=TRUE,warning=FALSE,message=F,fig.height=3,fig.width=4.89}
...
ggplot(data=variaciones_totales,aes(x=periodo,y=value,group=variables,color=variables))+geom_line(size=1)+
...
De los 14 dominios de departamentos representado por la encuesta, `r paste0(sum(tabla1$varprod>=0))` registraron variaciones positivas en su producción real, sumando `r round(sum(tabla1$varprod[tabla1$varprod>=0]),1)` p.p. a la variación total nacional.\
\
...
```{r tabla4_view,echo=F,eval=TRUE,warning=FALSE,message=F}
bar_chart <- function(label, height = "16px", fill = "#00bfc4", background = "white") {
    bar <- glue::glue("<div style='background:{fill};width:{label}%;height:{height};'></div>")
    chart <- glue::glue("<div style='flex-grow:1;margin-left:2px;background:{background};'>{bar}</div>")
    glue::glue("<div style='display:flex;align-items:left';>{chart}</div>") %>%
...
```{r tabla5_view,echo=F,eval=TRUE,warning=FALSE,message=F}
bar_chart <- function(label, height = "16px", fill = "#00bfc4", background = "white") {
    bar <- glue::glue("<div style='background:{fill};width:{label}%;height:{height};'></div>")
    chart <- glue::glue("<div style='flex-grow:1;margin-left:2px;background:{background};'>{bar}</div>")
    glue::glue("<div style='display:flex;align-items:left';>{chart}</div>") %>%
...
En `r paste0(meses_b[params$month]," de ",params$year)` frente a `r paste0(meses_b[params$month]," de ",params$year-1)` de los 10 dominios de ciudades representados por la encuesta, `r sum(tabla1$varprod>=0)` registraron variaciones positivas en su producción real, sumando `r round(sum(tabla1$varprod[tabla1$varprod>=0])*100,1)` p.p. de la variación total nacional (tabla 6)\
\
...
De los 14 dominios de departamentos representado por la encuesta,`r sum(tabla1$varprod>=0)` registraton variaciones positivas en su producción real, sumando `r round(sum(tabla1$varprod[tabla1$varprod>=0])*100,1)` p.p. a la variación total nacional (Tabla 7)\
\
...
En `r paste0(meses_b[params$month]," de ",params$year)` frente al mismo mes de 2019, la producción real de la industria manufacturera presentó una variación de frente al mismo mes de 2019, la producción real de la industria manufacturera presentó una variación de `r paste0(round(variaciones$variacion[variaciones$variable%in%c("Producción\nReal")]*100,1)," %")`, las ventas reales de `r paste0(round(variaciones$variacion[variaciones$variable%in%c("Ventas\nReales")]*100,1)," %")` y el personal ocupado   `r paste0(round(variaciones$variacion[variaciones$variable%in%c("Personal\nOcupado")]*100,1)," %")`.\
\
...
En `r paste0(meses_b[params$month]," de ",params$year)` frente al mismo mes de 2019 de los 10 dominios de ciudades representados por la encuesta, `r sum(tabla1$varprod>=0)` registraron variaciones positivas en su producción real, sumando `r round(sum(tabla1$varprod[tabla1$varprod>=0])*100,1)` p.p. de la variación total nacional (tabla 16)\
\
...
\begin{align*}
ICProducción &= `r paste0(params$IC_prod*100,"0%")`\%\\
ICVentas &= 98,1\%\\
ICEmpleo &= 98,5\%
\end{align*}
